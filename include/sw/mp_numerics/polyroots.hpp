...
//
// The three-stage Jenkins-Traub algorithm for all roots of a degree-n polynomial
// with real coefficients (J. Jenkins & J. Traub, "A Three-Stage Algorithm for
...
// float, posit, cfloat, ... and the study can observe how the working precision
// limits achievable root accuracy (especially for clustered / ill-separated
// roots).
//
// Migrated and modernized from Universal mixedprecision/roots/rpoly.cpp
// (universal#1204, #1211): the removed <blas/...> + numeric/containers types are
// replaced with MTL5's mtl::vec::dense_vector (which provides the same
// element-wise vector/scalar operators and a value-initialized size ctor), the
// mutable file-scope tolerance globals with immutable constants, and a
// value-returning poly_roots() API added. Complex roots use
// sw::universal::complex<Real> (std::complex is undefined behavior for the
...
#include <mtl/vec/operators.hpp>          // scalar*vector, vector/scalar
#include <universal/math/complex.hpp>

namespace sw::mp_numerics {

// Stopping-criterion tolerances (Ward's method). Double-scaled defaults tuned for
// the classic algorithm; achievable accuracy is ultimately bounded by the working
// precision `Real`, which the precision study varies. (Immutable -- replaces the
// original mutable file-scope `static double` globals.)
inline constexpr double kAbsoluteTolerance = 1e-14;
...
template <typename Real>
mtl::vec::dense_vector<Real> Head(const mtl::vec::dense_vector<Real>& vector, size_t size) {
    mtl::vec::dense_vector<Real> head(size);
...
template <typename Real>
mtl::vec::dense_vector<Real> Tail(const mtl::vec::dense_vector<Real>& vector, size_t size){
    mtl::vec::dense_vector<Real> tail(size);
...
template <typename Real>
inline Real EvaluatePolynomial(const mtl::vec::dense_vector<Real>& polynomial, const Real& x) {
    Real v = 0.0;
...
template <typename Real>
inline Complex<Real> EvaluateComplexPolynomial(const mtl::vec::dense_vector<Real>& polynomial, const Complex<Real>& x) {
...
template <typename Real>
mtl::vec::dense_vector<Real> RemoveLeadingZeros(const mtl::vec::dense_vector<Real>& polynomial_in) {
  size_t i = 0;
...
template <typename Real>
mtl::vec::dense_vector<Real> DifferentiatePolynomial(const mtl::vec::dense_vector<Real>& polynomial) {
    ...
    if (degree == 0) {
        return mtl::vec::dense_vector<Real>(1); // defaults to zero vector
    }

    mtl::vec::dense_vector<Real> derivative(degree);
...
template <typename Real>
mtl::vec::dense_vector<Real> MultiplyPolynomials(const mtl::vec::dense_vector<Real>& poly1, const mtl::vec::dense_vector<Real>& poly2) {
    mtl::vec::dense_vector<Real> multiplied_poly = mtl::vec::dense_vector<Real>(poly1.size() + poly2.size() - 1); // defaults to zero vector
...
template <typename Real>
mtl::vec::dense_vector<Real> AddPolynomials(const mtl::vec::dense_vector<Real>& poly1, const mtl::vec::dense_vector<Real>& poly2) {
    if (poly1.size() > poly2.size()) {
        mtl::vec::dense_vector<Real> sum = poly1;
        ...
        mtl::vec::dense_vector<Real> sum = poly2;
...
template <typename Real>
Real FindRootIterativeNewton(const mtl::vec::dense_vector<Real>& polynomial, const Real x0,
                             const Real epsilon, const int max_iterations) {
    using namespace sw::universal;
    using std::abs;
    
    Real root = x0;
    const mtl::vec::dense_vector<Real> derivative = DifferentiatePolynomial(polynomial);
    Real prev;
...
template <typename Real>
void SyntheticDivisionAndEvaluate(const mtl::vec::dense_vector<Real>& polynomial,
                                  const Real x,
                                  mtl::vec::dense_vector<Real>& quotient,
                                  Real& eval) {
    quotient = mtl::vec::dense_vector<Real>(polynomial.size() - 1, Real(0)); 
...
template <typename Real>
void QuadraticSyntheticDivision(const mtl::vec::dense_vector<Real>& polynomial,
                                const mtl::vec::dense_vector<Real>& quadratic_divisor,
                                mtl::vec::dense_vector<Real>& quotient,
                                mtl::vec::dense_vector<Real>& remainder) {
    
    using namespace sw::universal;

    quotient = mtl::vec::dense_vector<Real>(polynomial.size() - 2, Real(0));
    remainder = mtl::vec::dense_vector<Real>(2, Real(0));
...
template <typename Real>
bool HasConverged(const mtl::vec::dense_vector<Real>& sequence) { // TODO: could probably just be a normal vector, just for storing data so...
    using namespace sw::universal;
    using std::abs;
...
public:
    JenkinsTraubSolver(const mtl::vec::dense_vector<Real>& coeffs, mtl::vec::dense_vector<Real>& real_roots, mtl::vec::dense_vector<Real>& complex_roots)
        : polynomial_(coeffs), real_roots_(real_roots), complex_roots_(complex_roots), num_solved_roots_(0) 
        { sigma_ = mtl::vec::dense_vector<Real>(3); }
    ...
    bool ExtractRoots() {
        ...
        // normalize to monic; copy the leading coeff first so /= does not alias it
        const Real leading = polynomial_[0];
        polynomial_ /= leading;
        const int degree = polynomial_.size() - 1;

        // Allocate the output roots.
        real_roots_ = mtl::vec::dense_vector<Real>(degree, Real(0));
        complex_roots_ = mtl::vec::dense_vector<Real>(degree, Real(0));
...
private:
    ...
    void RemoveZeroRoots() {
        ...
        polynomial_ = Head(polynomial_, polynomial_.size() - num_zero_roots);  // drop the k zero-root (trailing) coeffs
    }
    ...
    Real ComputeRootRadius() {
        ...
        mtl::vec::dense_vector<Real> poly = polynomial_;
    ...
    void ComputeZeroShiftKPolynomial() {
        ...
        mtl::vec::dense_vector<Real> scaled_p = Head(polynomial_, polynomial_.size() - 1);
        scaled_p *= (-k_at_zero / polynomial_at_zero);
        k_polynomial_ = AddPolynomials(Head(k_polynomial_, k_polynomial_.size() - 1), scaled_p);
    }
    ...
    void ApplyZeroShiftToKPolynomial(const int num_iterations) {
        ...
        k_polynomial_ /= Real(polynomial_.size());
    ...
    void ComputeNextSigma(mtl::vec::dense_vector<Real>& nextSigma) {
    ...
    void UpdateKPolynomialWithQuadraticShift(
        const mtl::vec::dense_vector<Real>& polynomial_quotient,
        const mtl::vec::dense_vector<Real>& k_polynomial_quotient) {

        const Real coefficient_q_k = (a_ * a_ + sigma_[1] * a_ * b_ + sigma_[2] * b_ * b_) / (b_ * c_ - a_ * d_); // TODO: next segfault here
        mtl::vec::dense_vector<Real> linear_polynomial(2);
        linear_polynomial[0] = 1.0;
        linear_polynomial[1] = -(a_ * c_ + sigma_[1] * a_ * d_ + sigma_[2] * b_ * d_) / (b_ * c_ - a_ * d_);

        mtl::vec::dense_vector<Real> scaled_kq = k_polynomial_quotient;
        scaled_kq *= coefficient_q_k;
        k_polynomial_ = AddPolynomials(scaled_kq, MultiplyPolynomials(linear_polynomial, polynomial_quotient));
    ...
    ConvergenceType ApplyFixedShiftToKPolynomial(const Complex<Real>& root, const int max_iterations) {
        ...
        mtl::vec::dense_vector<Real> polynomial_quotient, polynomial_remainder;
        ...
        mtl::vec::dense_vector<Complex<Real>> t_lambda(3);
        mtl::vec::dense_vector<Real> sigma_lambda(3);
        mtl::vec::dense_vector<Real> k_polynomial_quotient, k_polynomial_remainder;
        for (int i = 0; i < max_iterations; ++i) {
            ...
            mtl::vec::dense_vector<Real> variable_shift_sigma(3);
    ...
    bool ApplyQuadraticShiftToKPolynomial(const Complex<Real>& root, const int max_iterations) {
        ...
        // zero index is the convergence value from 2 iterations ago, the first
        // index is from one iteration ago, and the second index is the current value.
        mtl::vec::dense_vector<Real> polynomial_quotient, polynomial_remainder, k_polynomial_quotient, k_polynomial_remainder;
    ...
    bool ApplyLinearShiftToKPolynomial(const Complex<Real>& root, const int max_iterations) {
        ...
        mtl::vec::dense_vector<Real> deflated_polynomial, deflated_k_polynomial;
        ...
        for (int i = 0; i < max_iterations; ++i) {
            ...
            mtl::vec::dense_vector<Real> scaled_dp = deflated_polynomial;
            scaled_dp *= (-k_polynomial_at_root / polynomial_at_root);
            k_polynomial_ = AddPolynomials(deflated_k_polynomial, scaled_dp);
    ...
    // and deflated.
    mtl::vec::dense_vector<Real> polynomial_;
    mtl::vec::dense_vector<Real> k_polynomial_;
    // Sigma is the quadratic factor the divides the K-polynomial.
    mtl::vec::dense_vector<Real> sigma_;
    ...
    mtl::vec::dense_vector<Real>& real_roots_;
    mtl::vec::dense_vector<Real>& complex_roots_;
...
template <typename Real>
bool FindPolynomialRootsJenkinsTraub(const mtl::vec::dense_vector<Real>& polynomial, mtl::vec::dense_vector<Real>& real_roots, mtl::vec::dense_vector<Real>& complex_roots) {
...
struct poly_roots_result {
    ...
    bool ok = false;                    ///< false if the leading coefficient was zero / extraction failed
};
...
[[nodiscard]] poly_roots_result<Real> poly_roots(const mtl::vec::dense_vector<Real>& coefficients) {
    ...
    mtl::vec::dense_vector<Real> real_parts, imag_parts;
    out.ok = FindPolynomialRootsJenkinsTraub(coefficients, real_parts, imag_parts);
    out.roots.reserve(real_parts.size());
    for (std::size_t i = 0; i < real_parts.size(); ++i)
        out.roots.emplace_back(real_parts[i], imag_parts[i]);
    return out;
}
...
[[nodiscard]] mtl::vec::dense_vector<Real> poly_from_real_roots(const std::vector<Real>& roots) {
    mtl::vec::dense_vector<Real> p{ Real(1) };
    for (const Real& r : roots)
        p = MultiplyPolynomials(p, mtl::vec::dense_vector<Real>{ Real(1), Real(-r) });
    return p;
}
